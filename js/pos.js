document.addEventListener('DOMContentLoaded', () => {
    ...
    function getEmployeeNameById(id) {
        ...
        return employee ? employee.name : 'ไม่พบชื่อ';
    }
    ...
    // --- All other functions (addToCart, modals, processSale, etc.) are here ---
    function renderCart() { if (cart.length === 0) { cartItemsContainer.innerHTML = `<p class="empty-cart-text">ตะกร้าสินค้าว่าง</p>`; checkoutButton.disabled = true; } else { cartItemsContainer.innerHTML = ''; cart.forEach(item => { const cartItemEl = document.createElement('div'); cartItemEl.className = 'cart-item'; cartItemEl.innerHTML = `<span>${item.name} (${item.price}฿) x${item.quantity}</span><span>฿${item.price * item.quantity}</span>`; cartItemsContainer.appendChild(cartItemEl); }); checkoutButton.disabled = false; } const totalPrice = cart.reduce((sum, item) => sum + (item.price * item.quantity), 0); totalPriceEl.textContent = `฿${totalPrice}`; }
    function addToCart(productId, price) { const product = products.find(p => p.id === productId); if (!product) return; const stock = liveStocks[productId] ?? 0; if (stock <= 0) { alert('สินค้าหมดสต็อก!'); return; } const cartItemId = `${productId}-${price}`; const existingItem = cart.find(item => item.cartId === cartItemId); if (existingItem) { if (existingItem.quantity < stock) { existingItem.quantity++; } else { alert('ไม่สามารถเพิ่มได้เนื่องจากสต็อกไม่เพียงพอ'); return; } } else { cart.push({ ...product, price: price, quantity: 1, cartId: cartItemId }); } renderCart(); }
    function openPriceModal(product) { document.getElementById('modal-product-name').textContent = `เลือกราคาสำหรับ: ${product.name}`; const optionsContainer = document.getElementById('modal-price-options'); optionsContainer.innerHTML = ''; const pricesToDisplay = product.prices || []; pricesToDisplay.forEach(price => { const button = document.createElement('button'); button.className = 'price-option-button'; button.textContent = `฿${price}`; button.onclick = () => { addToCart(product.id, price); closePriceModal(); }; optionsContainer.appendChild(button); }); priceModal.style.display = 'flex'; }
    ...
    function openChangeModal() { const total = cart.reduce((sum, item) => sum + (item.price * item.quantity), 0); document.getElementById('change-modal-total').textContent = `฿${total}`; moneyReceivedInput.value = ''; changeDueAmountEl.textContent = `฿0`; changeModal.style.display = 'flex'; moneyReceivedInput.focus(); }
    function closeChangeModal() { changeModal.style.display = 'none'; }
    async function fetchStockFromSupa() { const { data, error } = await supabaseClient.from('product_stocks').select('*'); if (error) { console.error('โหลด stock ล้มเหลว:', error.message); return false; } data.forEach(item => { liveStocks[item.product_id] = item.stock; }); return true; }
    supabaseClient.channel('stock-changes').on('postgres_changes', { event: '*', schema: 'public', table: 'product_stocks' }, (payload) => { const updated = payload.new; if (updated) { liveStocks[updated.product_id] = updated.stock; const activeTab = document.querySelector('.nav-item.active')?.dataset.tab; if (activeTab === 'sell-view') { const currentCategory = document.querySelector('.category-tab.active')?.dataset.category; if (currentCategory) renderProducts(currentCategory); } else if (activeTab === 'stock-view') { renderStockManagementList(stockSearchInput.value); } } }).subscribe();
    async function processSale(paymentMethod) { if (cart.length === 0) return; closePaymentModal(); closeChangeModal(); let shiftId = localStorage.getItem('currentShiftId'); if (!shiftId) { try { const { data, error } = await supabaseClient.from('shifts').insert({ employee_id: currentUser.id }).select().single(); if (error) throw error; shiftId = data.id; localStorage.setItem('currentShiftId', shiftId); } catch (error) { alert('เกิดข้อผิดพลาดในการเริ่มกะ: ' + error.message); return; } } const transactionId = crypto.randomUUID(); try { const saleRecords = cart.map(item => ({ transaction_id: transactionId, shift_id: shiftId, employee_id: currentUser.id, product_id: item.id, price: item.price, qty: item.quantity, payment_type: paymentMethod })); const { error } = await supabaseClient.from('sales').insert(saleRecords); if (error) throw error; } catch (error) { alert('เกิดข้อผิดพลาดในการบันทึกการขาย: ' + error.message); return; } try { const stockUpdates = cart.map(item => { const newStock = (liveStocks[item.id] || 0) - item.quantity; return supabaseClient.from('product_stocks').update({ stock: newStock }).eq('product_id', item.id); }); await Promise.all(stockUpdates); } catch (error) { alert('เกิดข้อผิดพลาดในการอัปเดตสต็อก: ' + error.message); return; } alert('บันทึกการขายสำเร็จ!'); cart = []; renderCart(); }
    async function fetchSalesHistory() { const today = new Date(); if (today.getHours() < 6) { today.setDate(today.getDate() - 1); } today.setHours(6, 0, 0, 0); const tomorrow = new Date(today); tomorrow.setDate(tomorrow.getDate() + 1); tomorrow.setHours(2, 0, 0, 0); const { data, error } = await supabaseClient.from('sales').select('*').gte('created_at', today.toISOString()).lt('created_at', tomorrow.toISOString()).order('created_at', { ascending: false }); if (error) { console.error('Error fetching sales history:', error); alert('ไม่สามารถโหลดประวัติการขายได้: ' + error.message); return null; } const groupedSales = data.reduce((acc, sale) => { const txId = sale.transaction_id; if (!acc[txId]) { acc[txId] = { items: [], total: 0, payment_type: sale.payment_type, employee_id: sale.employee_id, created_at: sale.created_at }; } acc[txId].items.push(sale); acc[txId].total += sale.price * sale.qty; return acc; }, {}); return Object.values(groupedSales); }
    async function displaySalesHistory() { loadingOverlay.style.display = 'flex'; const minLoadingTime = new Promise(resolve => setTimeout(resolve, 500)); try { const fetchPromise = fetchSalesHistory(); const [_, transactions] = await Promise.all([minLoadingTime, fetchPromise]); if (transactions) { renderSalesHistory(transactions); } } catch (error) { console.error("An error occurred in displaySalesHistory:", error); alert("เกิดข้อผิดพลาดในการแสดงผลประวัติการขาย"); } finally { loadingOverlay.style.display = 'none'; } }
    async function endCurrentShift() { if (!confirm('คุณต้องการปิดกะและสรุปยอดขายใช่หรือไม่?')) return; const shiftId = localStorage.getItem('currentShiftId'); if (!shiftId) { alert('ยังไม่มีการขายเกิดขึ้นในกะนี้ ไม่สามารถสรุปยอดได้'); return; } loadingOverlay.style.display = 'flex'; try { const { data: shiftStatus, error: statusError } = await supabaseClient.from('shifts').select('end_time').eq('id', shiftId).single(); if (statusError) throw new Error('ไม่พบข้อมูลกะปัจจุบันในระบบ อาจถูกลบไปแล้ว'); if (shiftStatus.end_time) { alert('กะนี้ได้ถูกปิดไปแล้ว'); localStorage.removeItem('currentShiftId'); loadingOverlay.style.display = 'none'; return; } const { data: sales, error: salesError } = await supabaseClient.from('sales').select('*').eq('shift_id', shiftId); if (salesError) throw salesError; const summary = sales.reduce((acc, sale) => { const saleTotal = sale.price * sale.qty; acc.total += saleTotal; if (sale.payment_type === 'cash') { acc.cash += saleTotal; } else if (sale.payment_type === 'transfer') { acc.transfer += saleTotal; } return acc; }, { total: 0, cash: 0, transfer: 0 }); const { error: updateError } = await supabaseClient.from('shifts').update({ end_time: new Date().toISOString(), summary_totals: summary }).eq('id', shiftId); if (updateError) throw updateError; document.getElementById('shift-total-sales').textContent = `฿${summary.total.toFixed(2)}`; document.getElementById('shift-cash-sales').textContent = `฿${summary.cash.toFixed(2)}`; document.getElementById('shift-transfer-sales').textContent = `฿${summary.transfer.toFixed(2)}`; shiftSummaryModal.style.display = 'flex'; } catch (error) { alert('เกิดข้อผิดพลาดในการปิดกะ: ' + error.message); } finally { loadingOverlay.style.display = 'none'; } }
    ...
    productCategorySelect.addEventListener('change', async (e) => { if (!isEditMode) { const categoryName = e.target.options[e.target.selectedIndex].text; const category = getCategoryByName(categoryName); if (category) { productIdInput.value = 'กำลังสร้าง...'; productIdInput.value = await generateNewProductId(category.id); } } });
